...
- (void)viewWillAppear:(BOOL)animated{
    _isFilterd = YES;
    NSError *error;
    NSData *savedTodos = [_defaults objectForKey:@"done"];
    ...
    _doneTasks = (NSMutableArray*)[NSKeyedUnarchiver unarchivedObjectOfClasses:set fromData:savedTodos error: &error];
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        case 0:
            //low
            image= [self getTaskImage:_isFilterd?[_lowTasks[indexPath.row] priority]:[_doneTasks[indexPath.row] priority]];
            cell.taskTitle.text = _isFilterd?[_lowTasks[indexPath.row] title]:[_doneTasks[indexPath.row] title];
            break;
        case 1:
            //mid
            image= [self getTaskImage:_isFilterd?[_medumTasks[indexPath.row] priority]:[_doneTasks[indexPath.row] priority]];
            cell.taskTitle.text = _isFilterd?[_medumTasks[indexPath.row] title]:[_doneTasks[indexPath.row] title];
            break;
        default:
            //high
            image= [self getTaskImage:_isFilterd?[_highTasks[indexPath.row] priority]:[_doneTasks[indexPath.row] priority]];
            cell.taskTitle.text = _isFilterd?[_highTasks[indexPath.row] title]:[_doneTasks[indexPath.row] title];
            break;
    }
...
-(void) deleteTask: (NSInteger) section :(NSInteger) row{
    if(_isFilterd){
        ...
        _doneTasks = [NSMutableArray new];
        [_doneTasks addObjectsFromArray:_lowTasks];
        [_doneTasks addObjectsFromArray:_medumTasks];
        [_doneTasks addObjectsFromArray:_highTasks];
    }else{
        [_doneTasks removeObjectAtIndex:row];
    }
...
-(void) updateDoneList{
    NSError *error;
    NSData *archiveData = [NSKeyedArchiver archivedDataWithRootObject:_doneTasks requiringSecureCoding:YES error:&error];
    
    [self devideArrayTasks];

    [_defaults setObject:archiveData forKey:@"done"];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    detailsScreen.status = 3;
    
    detailsScreen.tasks = _doneTasks;
    
    if(_isFilterd){
        switch (indexPath.section) {
            case 0:
                detailsScreen.taskIndex = [_doneTasks indexOfObject:_lowTasks[indexPath.row]];
                break;
            case 1:
                detailsScreen.taskIndex = [_doneTasks indexOfObject:_medumTasks[indexPath.row]];
                break;
            default:
                detailsScreen.taskIndex = [_doneTasks indexOfObject:_highTasks[indexPath.row]];
                break;
        }
    }else{
